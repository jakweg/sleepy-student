...
export const ALLOWED_CHANNELS = Object.freeze(
  (process.env.ALLOWED_CHANNELS || "").split(",").map((e) => e.trim())
);
export const WEBEX_NAME = process.env.WEBEX_NAME || "Wojtek";
export const WEBEX_MAIL = process.env.WEBEX_MAIL || "";
export const BARKA_CHANNEL_ID = process.env.BARKA_CHANNEL_ID || "";
export const BARKA_DURATION =
  parseInt(process.env.BARKA_DURATION_SECONDS) || 60;
export const RECORDING_READY_URL_FORMAT =
  process.env.RECORDING_READY_URL_FORMAT;
...
export const MS_TEAMS_CREDENTIALS_ORIGINS = Object.freeze(
  (process.env.TEAMS_ORIGINS || "").split(",").map((e) => e.trim())
);
export const MAX_MEETING_DURATION_MINUTES =
  parseInt(process.env.MAX_MEETING_DURATION, 10) || 90;
...
console.log(`Using config:
    ...
    WIDTH=${WIDTH}
    HEIGHT=${HEIGHT}
    FINAL_FILENAME_FORMAT=${FINAL_FILENAME_FORMAT}
    CRF=${VIDEO_CRF}
    FRAMERATE=${FRAMERATE}
