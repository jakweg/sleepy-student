...
export const joiningMessage = (
  ...
  `Elo <@${scheduledBy}>! Wchodzę na ${formatMeetingName(
    meetingName
  )} jak świnia w trufle`;

export const currentlyRecording = (name: string) =>
  `Leżę w kapitolu z 25-ma studentami, ${formatMeetingName(
    name
  )} leci 10 slajdy się psują? I UJ . Jakiś chyba?? wykładowca zabrania nagrywać ??! Warun idzie dostać. Trzymajcie się na tym wykładzie !!!`;

export const recordingFinished = (name: string) =>
  `Wykładu ${formatMeetingName(name)} już z nami nie ma, miał piękny pogrzeb`;

export const ENTER_MEETING_YOURSELF = `Dołącz do wesołego kółeczka samemu`;

export const MAY_NEED_YOUR_HELP_WITH_CAPTCHA =
  "Mogę potrzebować Twojej pomocy z kapczą";
...
export const recordingFinishedBy = (by: string) =>
  `Nagrywanie zatrzymane przez naszego kompana niedoli <@${by}>`;
export const recordingFinishedTimeout = (afterMinutes: number) =>
  `Koszmar studentów trwał ${afterMinutes} minut. Wykładowca nie chciał nas puścić. Donosił tylko slajdy od których studentom robiły się wielkie oczy`;
...
export const COMMAND_UPCOMING_DESCRIPTION =
  "Zobacz na jakie spotkania będę wkrótce wbijał";
